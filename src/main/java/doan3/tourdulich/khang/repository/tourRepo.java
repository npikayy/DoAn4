...
public interface tourRepo extends JpaRepository<tours, String> {
    ...
    @Query("SELECT t FROM tours t where t.tour_discount >= 50 order by t.tour_discount DESC LIMIT 3")
    public List<tours> find3Tour_discount();

    @Query("SELECT t FROM tours t where t.tour_discount >= 50")
